...
object ExpressionEvaluator {
	...
	private fun tokenize(expr: String): List<Token> {
		...
		while (i < s.length) {
			...
			when {
				c.isDigit() || c == '.' -> {
					...
					out += Token.NumberToken(s.substring(i, j).toDouble())
					i = j
					continue
				}
		...
		// Convert unary leading minus to unary function neg
		return normalizeUnaryMinus(out)
	}

	private fun normalizeUnaryMinus(tokens: List<Token>): List<Token> {
		val out = mutableListOf<Token>()
		var prev: Token? = null
		for (t in tokens) {
			if (t is Token.OperatorToken && t.op == "-") {
				if (prev == null || prev is Token.OperatorToken || prev is Token.LParen) {
					out += Token.FunctionToken("neg")
					prev = t
					continue
				}
			}
	...
	private fun evalRpn(tokens: List<Token>, degrees: Boolean): Double {
		...
		for (t in tokens) {
			when (t) {
				...
				is Token.OperatorToken -> {
					require(stack.size >= 2) { "Missing operand" }
					val b = stack.pop()
					val a = stack.pop()
					val res = when (t.op) {
						"+" -> a + b
